...
const Sidebar = ({selectedUser,setSelectedUser}) => {
  ...
  return (
    <div className= {`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-x-scroll text-white ${selectedUser ? "max-md:hidden" : ""}`}>
        <div className='pb-5'>
            <div className='flex justify-between items-center'>
                <img src={assets.logo} className='max-w-40'/>
                <div className='relative py-2 group'>
                    <img src={assets.menu_icon} className='max-h-5 cursor-pointer'/>
                    <div className='absolute top-full right-0 z-10 w-32 p-5 rounded-md 
        ...
        <div className='flex flex-col'>
            {userDummyData.map((user,idx) =>  (
                <div key={idx} onClick={() => {setSelectedUser(user)}} className= {`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm %{selectedUser ?._id === user._id && "bg-[#282142]/50"}`}>
